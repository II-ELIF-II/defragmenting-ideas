  ...
  return(
    <>
      <ul className="flex grow flex-col lg:flex-row h-full -mt-[12vh] md:mt-0 z-10 overflow-hidden snap-start">
        { (Posts.length != 0) ? Posts.map((post: postSummaryParams, index: number) => (<PostCardComp key={post.id} Post={post} Index={index}/>)) : <div className="absolute flex w-screen h-screen z-50 pointer-events-none"><ErrorCardComp error={'404 nothing found'} errorMessage={'There seems to be no information that matches the provided search parameters.'}/></div>}
        { fillerComps }
      </ul>
