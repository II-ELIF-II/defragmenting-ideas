...
const PaginatorComp = ({Page, Pages}: {Page: number, Pages: number}) => {
  ...
  return(
    <div className="relative w-full h-[6vh] flex justify-center items-center">
      <div className="flex justify-center items-center z-10 animate-slideInBottom">
        ...
        <button disabled={disablePrev} onClick={() => router.push(urlParamHandler({path: '?' + searchParams, param: 'page', value: Number(Page) - 1}), {scroll: false})} className={buttonNextAndPrevClass + " border-t border-r"}>
          <p className="flex justify-center items-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-5">
        ...
        {pages.map((page: Number) => (
          <button key={Number(page)} disabled={Page==page} onClick={() => router.push(urlParamHandler({path: '?' + searchParams, param: 'page', value: Number(page)}), {scroll: false})} className={buttonPageClass}>
            <p>{page.toString()}</p>
          </button>
        ))}

        <button disabled={disableNext} onClick={() => router.push(urlParamHandler({path: '?' + searchParams, param: 'page', value: Number(Page) + 1}), {scroll: false})} className={buttonNextAndPrevClass + " border-t border-l"}>
          <p className="flex justify-center items-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-5">
        ...
        <button disabled={disableNext} onClick={() => router.push(urlParamHandler({path: '?' + searchParams, param: 'page', value: Number(Pages)}), {scroll: false})}className={buttonNextAndPrevClass + " border-t"}>
          <p className="flex justify-center items-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-5">
