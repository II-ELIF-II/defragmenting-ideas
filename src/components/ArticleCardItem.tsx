...
export function ArticleCardItem(article: ArticleProps) {
  return (<div className="relative flex flex-col grow group overflow-hidden h-screen lg:h-full snap-center lg:snap-align-none ease-in-out duration-500 hover:z-10 hover:shadow-xl hover:shadow-neutral-950/60">
    ...
    <div className="h-full group-hover:animate-cameraMoves ease-in-out duration-500">
      <img src={article.thumbnail} draggable="false" className="object-cover h-full grow scale-105 group-hover:scale-125 ease-in-out duration-500"></img>
    </div>
    ...
    <div className="absolute bottom-0 flex flex-col w-full bg-gradient-to-r from-neutral-950/60 to-neutral-950/80 backdrop-blur-sm group-hover:backdrop-blur-md">
      <h1 className="text-md md:text-lg xl:text-xl px-3 py-1 bg-neutral-950/40"><span className="text-teal-400 font-semibold">&#92;&gt; </span>{article.title}</h1>
      <p className="text-xs text-neutral-400 px-3 mt-1 whitespace-pre-wrap">{GenerateUpdateTxt(article.createdAt, article.updatedAt)}</p>
      <p className="flex grow text-sm md:text-md xl:text-lg mb-1 px-3 text-justify">{article.summary}</p>
      <button onClick={() => setArticleState(true, article)} className="ml-auto mr-0 text-md px-2 py-1 text-right bg-teal-600 transition-all hover:pl-6 hover:bg-teal-400 active:bg-teal-700 flex items-center">Read more 
