...
const FeaturedThumbnailComp = ({postID, postThumb}: {postID: string, postThumb: string}) => {
  ...
  return(
    <div onClick={() => router.push(createPostUrl(postID))} className="flex h-full w-[200vw] ml-[-60vw] md:ml-0 md:w-full drop-shadow-lgCustom">
      ...
      <div className="hidden md:block absolute cursor-pointer h-full z-0 clip-right-slash-deeper animate-slideInRight bg-teal-700 w-full"></div>
    </div>
  )
}
